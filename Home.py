        ...
        #keyword_analysis = tab6.radio('Analysis:', ('Keyword in context', 'Collocation'))
		   ...
		   key='select_grid_2',
            ...
            # keyword = st.text_input('Enter a keyword:','staff')
                Word_type = st.selectbox('Choose word type:', ['All words', 'Nouns', 'Proper nouns', 'Verbs', 'Adjectives', 'Adverbs', 'Numbers'])
                collocs = get_collocs(kwic_instances)

                # Generate collocation string with frequency.
...
def plot_kwic(data, key,tab):
    ...
    try:
        with tab:
            ...
            keyword = st.selectbox('Select keyword: [note: the number following the listed words indicated how many times it appears in the data]', topwords).split('(',1)[0].strip()
            
            window_size = st.slider('Select the number of words to the right and left of your keyword:', 1, 10, 5)
